# Make tf.data.Datasets from 20CRv3 monthly UK fields
...
def getDataset(purpose, nImages=None):
    ...
    inFiles = os.listdir(
        "%s/ML_monthly_UK/datasets/%s" % (os.getenv("SCRATCH"), purpose)
    )
    ...
    inFiles = [
        "%s/ML_monthly_UK/datasets/%s/%s" % (os.getenv("SCRATCH"), purpose, x)
        for x in inFiles
    ]
