...
nImagesInEpoch = None
nRepeatsPerEpoch = 1  # Show each image this many times
...
def save_state(
    ...
    history["CO2_train"].append(CO2_train)
    history["CO2_test"].append(CO2_test)
    history["MONTH_train"].append(MONTH_train)
    history["MONTH_test"].append(MONTH_test)
...
def test_stats(autoencoder, tst_ds):
    ...
    rmse_CO2 = tf.keras.metrics.Mean()
    rmse_MONTH = tf.keras.metrics.Mean()
...
with strategy.scope():
    ...
    for epoch in range(args.epoch, nEpochs):
        ...
        print(
            "CO2    : {:>6.1f}, {:>6.1f}".format(
                train_rmse_CO2.result(), test_rmse_CO2.result()
            )
        )
        print(
